# Documentation: `crates/portfolio/src/portfolio.rs`
**Generated:** 2025-11-15T19:40:03.384320Z
**File Size:** 70474 bytes
**Extension:** .rs
**Type:** text
...
- **Path:** `crates/portfolio/src/portfolio.rs`
- **Size:** 70,474 bytes
- **Lines:** 1,992
- **Extension:** `.rs`
- **Type:** text
...
This file is located at `crates/portfolio/src/portfolio.rs` within the repository.

**Classes defined:** PortfolioState, Portfolio, PortfolioState, Debug, Portfolio

**Functions defined:** new, reset, fmt, new, register_message_handlers, reset, is_initialized, balances_locked, margins_init, margins_maint and 32 more
...
#### `unrealized_pnls(&mut self, venue: &Venue)`


#### `realized_pnls(&mut self, venue: &Venue)`


#### `net_exposures(&self, venue: &Venue)`


#### `unrealized_pnl(&mut self, instrument_id: &InstrumentId)`


#### `realized_pnl(&mut self, instrument_id: &InstrumentId)`


#### `total_pnl(&mut self, instrument_id: &InstrumentId)`


#### `total_pnls(&mut self, venue: &Venue)`


#### `net_exposure(&self, instrument_id: &InstrumentId)`


#### `net_position(&self, instrument_id: &InstrumentId)`


#### `is_net_long(&self, instrument_id: &InstrumentId)`


#### `is_net_short(&self, instrument_id: &InstrumentId)`


#### `is_flat(&self, instrument_id: &InstrumentId)`


#### `is_completely_flat(&self)`


#### `initialize_orders(&mut self)`


#### `initialize_positions(&mut self)`


#### `update_quote_tick(&mut self, quote: &QuoteTick)`


#### `update_bar(&mut self, bar: &Bar)`


#### `update_account(&mut self, event: &AccountState)`


#### `update_order(&mut self, event: &OrderEventAny)`


#### `update_position(&mut self, event: &PositionEvent)`


#### `update_net_position(&mut self, instrument_id: &InstrumentId, positions_open: Vec<Position>)`


#### `calculate_unrealized_pnl(&mut self, instrument_id: &InstrumentId)`


#### `ensure_snapshot_pnls_cached_for(&mut self, instrument_id: &InstrumentId)`


#### `calculate_realized_pnl(&mut self, instrument_id: &InstrumentId)`


#### `get_price(&self, position: &Position)`
