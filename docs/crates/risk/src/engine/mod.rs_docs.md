# Documentation: `crates/risk/src/engine/mod.rs`
**Generated:** 2025-11-15T19:40:03.409112Z
**File Size:** 49773 bytes
**Extension:** .rs
**Type:** text
...
- **Path:** `crates/risk/src/engine/mod.rs`
- **Size:** 49,773 bytes
- **Lines:** 1,261
- **Extension:** `.rs`
- **Type:** text
...
This file is located at `crates/risk/src/engine/mod.rs` within the repository.

**Classes defined:** RiskEngine, Debug, RiskEngine

**Functions defined:** fmt, new, create_submit_order_throttler, create_modify_order_throttler, handle_submit_order_cache, get_existing_order, create_order_denied, create_modify_rejected, execute, process and 19 more
...
#### `handle_submit_order_cache(cache: &Rc<RefCell<Cache>>, submit_order: &SubmitOrder)`


#### `get_existing_order(cache: &Rc<RefCell<Cache>>, order: &ModifyOrder)`
...
#### `execute(&mut self, command: TradingCommand)`


#### `process(&mut self, event: OrderEventAny)`


#### `set_trading_state(&mut self, state: TradingState)`


#### `set_max_notional_per_order(&mut self, instrument_id: InstrumentId, new_value: Decimal)`


#### `handle_command(&mut self, command: TradingCommand)`


#### `handle_submit_order(&mut self, command: SubmitOrder)`


#### `handle_submit_order_list(&mut self, command: SubmitOrderList)`


#### `handle_modify_order(&mut self, command: ModifyOrder)`


#### `check_order(&self, instrument: InstrumentAny, order: OrderAny)`


#### `check_order_price(&self, instrument: InstrumentAny, order: OrderAny)`


#### `check_order_quantity(&self, instrument: InstrumentAny, order: OrderAny)`


#### `check_orders_risk(&self, instrument: InstrumentAny, orders: Vec<OrderAny>)`


#### `check_price(&self, instrument: &InstrumentAny, price: Option<Price>)`
...
#### `deny_command(&self, command: TradingCommand, reason: &str)`


#### `deny_order(&self, order: OrderAny, reason: &str)`


#### `deny_order_list(&self, order_list: OrderList, reason: &str)`


#### `reject_modify_order(&self, order: OrderAny, reason: &str)`


#### `execution_gateway(&mut self, instrument: InstrumentAny, command: TradingCommand)`


#### `send_to_execution(&self, command: TradingCommand)`
