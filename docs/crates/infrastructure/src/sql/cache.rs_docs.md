# Documentation: `crates/infrastructure/src/sql/cache.rs`
**Generated:** 2025-11-15T19:40:02.336508Z
**File Size:** 40229 bytes
**Extension:** .rs
**Type:** text
...
This file is located at `crates/infrastructure/src/sql/cache.rs` within the repository.

**Classes defined:** PostgresCacheDatabase, PostgresCacheDatabase, CacheDatabaseAdapter

**Functions defined:** connect, process_commands, get_pg_cache_database, close, flush, load_all, load, load_currencies, load_instruments, load_synthetics and 51 more
...
#### `load_actor(&self, component_id: &ComponentId)`


#### `delete_actor(&self, component_id: &ComponentId)`


#### `load_strategy(&self, strategy_id: &StrategyId)`


#### `delete_strategy(&self, component_id: &StrategyId)`
...
#### `delete_account_event(&self, account_id: &AccountId, event_id: &str)`


#### `add(&self, key: String, value: Bytes)`
...
#### `add_account(&self, account: &AccountAny)`


#### `add_order(&self, order: &OrderAny, client_id: Option<ClientId>)`


#### `add_order_snapshot(&self, snapshot: &OrderSnapshot)`


#### `add_position(&self, position: &Position)`


#### `add_position_snapshot(&self, snapshot: &PositionSnapshot)`


#### `add_order_book(&self, order_book: &OrderBook)`


#### `add_quote(&self, quote: &QuoteTick)`


#### `load_quotes(&self, instrument_id: &InstrumentId)`


#### `add_trade(&self, trade: &TradeTick)`


#### `load_trades(&self, instrument_id: &InstrumentId)`


#### `add_bar(&self, bar: &Bar)`


#### `load_bars(&self, instrument_id: &InstrumentId)`


#### `add_signal(&self, signal: &Signal)`


#### `load_signals(&self, name: &str)`


#### `add_custom_data(&self, data: &CustomData)`


#### `load_custom_data(&self, data_type: &DataType)`
...
**Directory:** `crates/infrastructure/src/sql`
