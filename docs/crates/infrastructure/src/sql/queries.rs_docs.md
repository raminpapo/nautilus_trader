# Documentation: `crates/infrastructure/src/sql/queries.rs`
**Generated:** 2025-11-15T19:40:02.365981Z
**File Size:** 53488 bytes
**Extension:** .rs
**Type:** text
...
This file is located at `crates/infrastructure/src/sql/queries.rs` within the repository.

**Classes defined:** DatabaseQueries, DatabaseQueries

**Functions defined:** truncate, add, load, add_currency, load_currencies, load_currency, add_instrument, load_instrument, load_instruments, add_order and 25 more
...
#### `truncate(pool: &PgPool)`


#### `add(pool: &PgPool, key: String, value: Vec<u8>)`


#### `load(pool: &PgPool)`


#### `add_currency(pool: &PgPool, currency: Currency)`


#### `load_currencies(pool: &PgPool)`


#### `load_currency(pool: &PgPool, code: &str)`
...
#### `add_order_snapshot(pool: &PgPool, snapshot: OrderSnapshot)`
...
#### `load_accounts(pool: &PgPool)`


#### `add_trade(pool: &PgPool, trade: &TradeTick)`
...
#### `add_quote(pool: &PgPool, quote: &QuoteTick)`
...
#### `add_signal(pool: &PgPool, signal: &Signal)`


#### `load_signals(pool: &PgPool, name: &str)`


#### `add_custom_data(pool: &PgPool, data: &CustomData)`
