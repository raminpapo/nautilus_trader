# Documentation: `crates/infrastructure/src/redis/cache.rs`
**Generated:** 2025-11-15T19:40:02.320468Z
**File Size:** 40221 bytes
**Extension:** .rs
**Type:** text
...
This file is located at `crates/infrastructure/src/redis/cache.rs` within the repository.

**Classes defined:** DatabaseCommand, RedisCacheDatabase, RedisCacheDatabaseAdapter, DatabaseCommand, Debug, RedisCacheDatabase, CacheDatabaseAdapter

**Functions defined:** new, close, fmt, new, get_encoding, get_trader_key, close, flushdb, keys, read and 87 more
...
#### `new(op_type: DatabaseOperation, key: String, payload: Option<Vec<Bytes>>)`
...
#### `keys(&mut self, pattern: &str)`


#### `read(&mut self, key: &str)`


#### `read_bulk(&mut self, keys: &[String])`


#### `insert(&mut self, key: String, payload: Option<Vec<Bytes>>)`


#### `update(&mut self, key: String, payload: Option<Vec<Bytes>>)`


#### `delete(&mut self, key: String, payload: Option<Vec<Bytes>>)`


#### `delete_order(&self, client_order_id: &ClientOrderId)`


#### `delete_position(&self, position_id: &PositionId)`
...
#### `insert_index(pipe: &mut Pipeline, key: &str, value: &[Bytes])`


#### `insert_string(pipe: &mut Pipeline, key: &str, value: &[u8])`


#### `insert_set(pipe: &mut Pipeline, key: &str, value: &[u8])`


#### `insert_hset(pipe: &mut Pipeline, key: &str, name: &[u8], value: &[u8])`


#### `insert_list(pipe: &mut Pipeline, key: &str, value: &[u8])`
...
#### `update_list(pipe: &mut Pipeline, key: &str, value: &[u8])`
...
#### `remove_from_set(pipe: &mut Pipeline, key: &str, member: &[u8])`


#### `remove_from_hash(pipe: &mut Pipeline, key: &str, field: &[u8])`


#### `delete_string(pipe: &mut Pipeline, key: &str)`


#### `get_trader_key(trader_id: TraderId, instance_id: UUID4, config: &CacheConfig)`


#### `get_collection_key(key: &str)`


#### `get_index_key(key: &str)`
...
**Directory:** `crates/infrastructure/src/redis`
