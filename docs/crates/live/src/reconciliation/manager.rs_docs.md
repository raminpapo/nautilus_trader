# Documentation: `crates/live/src/reconciliation/manager.rs`
**Generated:** 2025-11-15T19:40:02.415487Z
**File Size:** 29109 bytes
**Extension:** .rs
**Type:** text
...
This file is located at `crates/live/src/reconciliation/manager.rs` within the repository.

**Classes defined:** ReconciliationConfig, ExecutionReport, InflightCheck, is, must, ReconciliationManager, Default, Debug, ReconciliationManager

**Functions defined:** default, fmt, new, reconcile_execution_mass_status, reconcile_report, check_inflight_orders, check_open_orders, register_inflight, record_local_activity, clear_recon_tracking and 23 more
...
#### `check_inflight_orders(&mut self)`


#### `check_open_orders(&mut self)`


#### `register_inflight(&mut self, client_order_id: ClientOrderId)`


#### `record_local_activity(&mut self, client_order_id: ClientOrderId, ts_event: UnixNanos)`


#### `clear_recon_tracking(&mut self, client_order_id: &ClientOrderId, drop_last_query: bool)`


#### `claim_external_orders(&mut self, instrument_id: InstrumentId, strategy_id: StrategyId)`


#### `get_order(&self, client_order_id: &ClientOrderId)`


#### `get_instrument(&self, instrument_id: &InstrumentId)`
...
#### `create_order_accepted(&self, order: &OrderAny, report: &OrderStatusReport)`


#### `create_order_rejected(&self, order: &OrderAny, reason: Option<&str>)`
...
#### `create_order_canceled(&self, order: &OrderAny, report: &OrderStatusReport)`


#### `create_order_canceled_simple(&self, order: &OrderAny, ts_event: UnixNanos)`


#### `create_order_expired(&self, order: &OrderAny, report: &OrderStatusReport)`
