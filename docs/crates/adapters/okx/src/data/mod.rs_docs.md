# Documentation: `crates/adapters/okx/src/data/mod.rs`
**Generated:** 2025-11-15T19:40:01.264948Z
...
- **Path:** `crates/adapters/okx/src/data/mod.rs`
- **Size:** 45,205 bytes
- **Lines:** 1,286
- **Extension:** `.rs`
- **Type:** text
...
This file is located at `crates/adapters/okx/src/data/mod.rs` within the repository.

**Classes defined:** OKXDataClient, OKXDataClient, futures_util, DataClient

**Functions defined:** new, venue, vip_level, public_ws, public_ws_mut, business_ws, business_ws_mut, send_data, spawn_ws, bootstrap_instruments and 43 more
...
#### `new(client_id: ClientId, config: OKXDataClientConfig)`
...
#### `public_ws_mut(&mut self)`


#### `business_ws(&self)`


#### `business_ws_mut(&mut self)`


#### `send_data(sender: &tokio::sync::mpsc::UnboundedSender<DataEvent>, data: Data)`


#### `spawn_ws(&self, fut: F, context: &'static str)`


#### `bootstrap_instruments(&mut self)`


#### `contract_filter(&self, instrument: &InstrumentAny)`
...
#### `spawn_public_stream(&mut self)`


#### `spawn_business_stream(&mut self)`
...
#### `maybe_spawn_instrument_refresh(&mut self)`


#### `emit_funding_rates(updates: Vec<FundingRateUpdate>)`
...
#### `datetime_to_unix_nanos(value: Option<DateTime<Utc>>)`


#### `contract_filter_with_config(config: &OKXDataClientConfig, instrument: &InstrumentAny)`


#### `contract_filter_with_config_types(
    contract_types: Option<&Vec<OKXContractType>>,
    instrument: &InstrumentAny,
)`
...
#### `subscribe_instrument(&mut self, cmd: &SubscribeInstrument)`


#### `subscribe_book_deltas(&mut self, cmd: &SubscribeBookDeltas)`


#### `subscribe_book_snapshots(&mut self, cmd: &SubscribeBookSnapshots)`


#### `subscribe_quotes(&mut self, cmd: &SubscribeQuotes)`


#### `subscribe_trades(&mut self, cmd: &SubscribeTrades)`


#### `subscribe_mark_prices(&mut self, cmd: &SubscribeMarkPrices)`


#### `subscribe_index_prices(&mut self, cmd: &SubscribeIndexPrices)`


#### `subscribe_funding_rates(&mut self, cmd: &SubscribeFundingRates)`


#### `subscribe_bars(&mut self, cmd: &SubscribeBars)`


#### `unsubscribe_book_deltas(&mut self, cmd: &UnsubscribeBookDeltas)`


#### `unsubscribe_book_snapshots(&mut self, cmd: &UnsubscribeBookSnapshots)`


#### `unsubscribe_quotes(&mut self, cmd: &UnsubscribeQuotes)`


#### `unsubscribe_trades(&mut self, cmd: &UnsubscribeTrades)`


#### `unsubscribe_mark_prices(&mut self, cmd: &UnsubscribeMarkPrices)`


#### `unsubscribe_index_prices(&mut self, cmd: &UnsubscribeIndexPrices)`


#### `unsubscribe_funding_rates(&mut self, cmd: &UnsubscribeFundingRates)`


#### `unsubscribe_bars(&mut self, cmd: &UnsubscribeBars)`
...
**Directory:** `crates/adapters/okx/src/data`
