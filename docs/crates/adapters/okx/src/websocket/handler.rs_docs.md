# Documentation: `crates/adapters/okx/src/websocket/handler.rs`
**Generated:** 2025-11-15T19:40:01.338130Z
**File Size:** 95479 bytes
**Extension:** .rs
**Type:** text
...
This file is located at `crates/adapters/okx/src/websocket/handler.rs` within the repository.

**Classes defined:** OKXWsFeedHandler, OKXWsFeedHandler

**Functions defined:** new, is_stopped, send, send_with_retry, send_pong, next, is_post_only_auto_cancel, try_handle_post_only_auto_cancel, register_client_order_aliases, adjust_execution_report and 28 more
...
#### `send(&self, msg: NautilusWsMessage)`
...
#### `is_post_only_auto_cancel(msg: &OKXOrderMsg)`


#### `try_handle_post_only_auto_cancel(
        &mut self,
        msg: &OKXOrderMsg,
        ts_init: UnixNanos,
        exec_reports: &mut Vec<ExecutionReport>,
...
#### `update_caches_with_report(&mut self, report: &ExecutionReport)`
...
#### `handle_positions_data(&mut self, data: Value, ts_init: UnixNanos)`


#### `handle_orders_data(&mut self, data: Value, ts_init: UnixNanos)`


#### `handle_algo_orders_data(
        &mut self,
        data: Value,
        ts_init: UnixNanos,
    )`
...
#### `get_instrument_type_and_family_from_instrument(
        instrument: &InstrumentAny,
    )`


#### `handle_mass_cancel(&mut self, instrument_id: InstrumentId)`
...
#### `handle_subscribe(&self, args: Vec<OKXSubscriptionArg>)`


#### `handle_unsubscribe(&self, args: Vec<OKXSubscriptionArg>)`
...
#### `is_post_only_rejection(code: &str, data: &[Value])`


#### `should_retry_okx_error(error: &OKXWsError)`


#### `create_okx_timeout_error(msg: String)`


#### `test_is_post_only_rejection_detects_by_code()`


#### `test_is_post_only_rejection_detects_by_inner_code()`


#### `test_is_post_only_rejection_false_for_unrelated_error()`
