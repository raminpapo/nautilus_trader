# Documentation: `crates/adapters/bybit/src/websocket/handler.rs`
**Generated:** 2025-11-15T19:40:00.687787Z
**File Size:** 59232 bytes
**Extension:** .rs
**Type:** text
...
impl FeedHandler {
    ...
    fn handle_batch_failure(
        ...
        if op.contains("create") {
            if let Some((_, batch_data)) = self.pending_batch_place_requests.remove(req_id) {
                ...
                for (client_order_id, (trader_id, strategy_id, instrument_id)) in batch_data {
            ...
            && let Some((_, batch_data)) = self.pending_batch_cancel_requests.remove(req_id)
        {
            ...
            let reason = Ustr::from(ret_msg);
            for (client_order_id, (trader_id, strategy_id, instrument_id, venue_order_id)) in
                batch_data
            {
    ...
    fn process_batch_place_errors(
        ...
        for (idx, (client_order_id, (trader_id, strategy_id, instrument_id))) in
    ...
    fn process_batch_cancel_errors(
        ...
        for (idx, (client_order_id, (trader_id, strategy_id, instrument_id, venue_order_id))) in
            batch_data.into_iter().enumerate()
        {
            if let Some(error) = errors.get(idx)
                && error.code != 0
    ...
    async fn parse_to_nautilus_messages(
        ...
        match msg {
            ...
            BybitWsMessage::OrderResponse(resp) => {
                // Try req_id field first, fall back to orderLinkId from data
                let request_id = resp.req_id.clone().or_else(|| {
                    resp.data
                        .get("orderLinkId")
                        .and_then(|v| v.as_str())
                        .map(String::from)
...
This file is located at `crates/adapters/bybit/src/websocket/handler.rs` within the repository.

**Classes defined:** FeedHandler, FeedHandler

**Functions defined:** new, is_stopped, send, send_with_retry, handle_batch_failure, handle_batch_response, process_batch_place_errors, process_batch_cancel_errors, next, parse_raw_message and 4 more
...
#### `send_with_retry(&self, payload: String)`
...
#### `parse_raw_message(msg: Message)`


#### `classify_bybit_message(value: &serde_json::Value)`
...
#### `should_retry_bybit_error(error: &BybitWsError)`


#### `create_bybit_timeout_error(msg: String)`
