# Documentation: `crates/adapters/hyperliquid/src/websocket/post.rs`
**Generated:** 2025-11-15T19:40:01.102391Z
**File Size:** 33249 bytes
**Extension:** .rs
**Type:** text
...
- **Path:** `crates/adapters/hyperliquid/src/websocket/post.rs`
- **Size:** 33,249 bytes
- **Lines:** 1,036
- **Extension:** `.rs`
- **Type:** text
...
This file is located at `crates/adapters/hyperliquid/src/websocket/post.rs` within the repository.

**Classes defined:** Waiter, PostRouter, PostIds, ScheduledPost, PostBatcher, LimitOrderParams, TriggerOrderParams, OrderBuilder, pub, pub and 14 more

**Functions defined:** default, new, register, complete, cancel, await_with_timeout, new, next, new, run_lane and 42 more
...
#### `register(&self, id: u64)`


#### `complete(&self, resp: PostResponse)`
...
#### `enqueue(&self, item: ScheduledPost)`


#### `lane_for_action(action: &ActionRequest)`
...
#### `push_limit_order(mut self, params: LimitOrderParams)`
...
#### `push_trigger_order(mut self, params: TriggerOrderParams)`


#### `build(self)`


#### `single_limit_order(params: LimitOrderParams)`


#### `single_trigger_order(params: TriggerOrderParams)`


#### `cancel_many(cancels: Vec<(u32, u64)`


#### `cancel_by_cloid(asset: u32, cloid: impl Into<String>)`


#### `modify(oid: u64, new_order: OrderRequest)`


#### `info_l2_book(coin: &str)`


#### `info_all_mids()`


#### `info_order_status(user: &str, oid: u64)`


#### `info_open_orders(user: &str, frontend: Option<bool>)`


#### `info_user_fills(user: &str, aggregate_by_time: Option<bool>)`


#### `info_user_rate_limit(user: &str)`


#### `info_candle(coin: &str, interval: &str)`


#### `parse_l2_book(payload: &serde_json::Value)`


#### `parse_user_fills(payload: &serde_json::Value)`


#### `parse_order_status(payload: &serde_json::Value)`


#### `classify_action_payload(payload: &serde_json::Value)`


#### `new(tx: mpsc::Sender<HyperliquidWsRequest>)`


#### `send(&self, req: HyperliquidWsRequest)`


#### `mk_limit_alo(asset: u32)`


#### `mk_limit_gtc(asset: u32)`
...
#### `lane_classifier_cases(orders: Vec<OrderRequest>, expected: PostLane)`
