# Documentation: `crates/adapters/hyperliquid/src/signing/signers.rs`
**Generated:** 2025-11-15T19:40:01.070911Z
...
This file is located at `crates/adapters/hyperliquid/src/signing/signers.rs` within the repository.

**Classes defined:** for, Agent, SignRequest, SignatureBundle, HyperliquidEip712Signer, with, let, HyperliquidEip712Signer

**Functions defined:** new, sign, sign_l1_action, compute_connection_id, sign_user_signed_action, get_domain_hash, hash_typed_data, create_eip712_hash, sign_hash, canonicalize_action and 8 more
...
#### `new(private_key: EvmPrivateKey)`


#### `sign(&self, request: &SignRequest)`


#### `sign_l1_action(&self, request: &SignRequest)`


#### `compute_connection_id(&self, request: &SignRequest)`


#### `sign_user_signed_action(&self, action: &Value, _nonce: TimeNonce)`


#### `get_domain_hash(&self)`


#### `hash_typed_data(&self, data: &Value)`
...
#### `sign_hash(&self, hash: &[u8; 32])`


#### `canonicalize_action(action: &Value)`


#### `canonicalize_address(addr: &str)`


#### `canonicalize_decimal(decimal: &str)`
