# Documentation: `crates/adapters/hyperliquid/src/execution/mod.rs`
**Generated:** 2025-11-15T19:40:01.024402Z
**File Size:** 40935 bytes
**Extension:** .rs
**Type:** text
...
This file is located at `crates/adapters/hyperliquid/src/execution/mod.rs` within the repository.

**Classes defined:** HyperliquidExecutionClient, let, let, let, let, definition, HyperliquidExecutionClient, ExecutionClient, LiveExecutionClient, LiveExecutionClientExt and 1 more

**Functions defined:** config, validate_order_submission, new, ensure_instruments_initialized_async, ensure_instruments_initialized, refresh_account_state, get_user_address, spawn_task, abort_pending_tasks, update_account_state and 28 more
...
#### `validate_order_submission(&self, order: &OrderAny)`
...
#### `submit_order(&self, command: &SubmitOrder)`


#### `submit_order_list(&self, command: &SubmitOrderList)`


#### `modify_order(&self, command: &ModifyOrder)`


#### `cancel_order(&self, command: &CancelOrder)`


#### `cancel_all_orders(&self, command: &CancelAllOrders)`


#### `batch_cancel_orders(&self, command: &BatchCancelOrders)`


#### `query_account(&self, command: &QueryAccount)`


#### `query_order(&self, command: &QueryOrder)`
...
**Directory:** `crates/adapters/hyperliquid/src/execution`
...
**Security:** This file may contain sensitive patterns: secret, private_key, credential, auth. Ensure proper handling of secrets.
