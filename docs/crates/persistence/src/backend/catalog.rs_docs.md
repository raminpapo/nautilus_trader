# Documentation: `crates/persistence/src/backend/catalog.rs`
**Generated:** 2025-11-15T19:40:03.318399Z
**File Size:** 85239 bytes
**Extension:** .rs
**Type:** text
...
This file is located at `crates/persistence/src/backend/catalog.rs` within the repository.

**Classes defined:** ParquetDataCatalog, Debug, ParquetDataCatalog, CatalogPathPrefix

**Functions defined:** fmt, new, from_uri, get_base_path, reset_session, write_data_enum, write_to_parquet, write_to_json, check_ascending_timestamps, data_to_record_batches and 38 more
...
#### `check_ascending_timestamps(
        data: &[T],
        type_name: &str,
    )`


#### `data_to_record_batches(&self, data: Vec<T>)`
...
#### `list_parquet_files(&self, directory: &str)`


#### `reconstruct_full_uri(&self, path_str: &str)`


#### `join_paths(&self, base: &str, path: &str)`
...
#### `get_directory_intervals(&self, directory: &str)`
...
#### `to_object_path(&self, path: &str)`


#### `move_file(&self, old_path: &ObjectPath, new_path: &ObjectPath)`


#### `execute_async(&self, future: F)`
...
#### `timestamps_to_filename(timestamp_1: UnixNanos, timestamp_2: UnixNanos)`


#### `iso_timestamp_to_file_timestamp(iso_timestamp: &str)`


#### `file_timestamp_to_iso_timestamp(file_timestamp: &str)`


#### `iso_to_unix_nanos(iso_timestamp: &str)`


#### `urisafe_instrument_id(instrument_id: &str)`


#### `extract_identifier_from_path(file_path: &str)`


#### `make_sql_safe_identifier(identifier: &str)`


#### `extract_sql_safe_filename(file_path: &str)`


#### `make_object_store_path(base_path: &str, components: &[&str])`


#### `make_object_store_path_owned(base_path: &str, components: Vec<String>)`


#### `local_to_object_store_path(local_path: &Path)`


#### `extract_path_components(path_str: &str)`


#### `query_intersects_filename(filename: &str, start: Option<u64>, end: Option<u64>)`


#### `parse_filename_timestamps(filename: &str)`


#### `are_intervals_disjoint(intervals: &[(u64, u64)`


#### `are_intervals_contiguous(intervals: &[(u64, u64)`


#### `query_interval_diff(start: u64, end: u64, closed_intervals: &[(u64, u64)`


#### `get_interval_set(intervals: &[(u64, u64)`
...
**Directory:** `crates/persistence/src/backend`
...
**Security:** This file may contain sensitive patterns: secret, token, credential, auth, session. Ensure proper handling of secrets.
