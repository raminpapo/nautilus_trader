# Documentation: `crates/common/src/greeks.rs`
**Generated:** 2025-11-15T19:40:01.737116Z
**File Size:** 48517 bytes
**Extension:** .rs
**Type:** text
...
This file is located at `crates/common/src/greeks.rs` within the repository.

**Classes defined:** InstrumentGreeksParams, PortfolioGreeksParams, GreeksCalculator, compiles, GreeksFilterCallback, Debug, InstrumentGreeksParams, std, PortfolioGreeksParams, GreeksCalculator

**Functions defined:** from_fn, from_closure, call, to_greeks_filter, fmt, calculate, fmt, calculate, new, instrument_greeks and 30 more
...
#### `from_fn(f: fn(&GreeksData)`


#### `from_closure(f: F)`


#### `call(&self, data: &GreeksData)`


#### `to_greeks_filter(self)`


#### `fmt(&self, f: &mut std::fmt::Formatter<'_>)`


#### `calculate(&self, calculator: &GreeksCalculator)`
...
#### `new(cache: Rc<RefCell<Cache>>, clock: Rc<RefCell<dyn Clock>>)`
...
#### `subscribe_greeks(&self, underlying: &str, handler: Option<F>)`
...
#### `filter_positive_delta(data: &GreeksData)`
...
#### `filter_fn(data: &GreeksData)`


#### `test_portfolio_greeks_params_with_filter()`


#### `filter_high_delta(data: &GreeksData)`
