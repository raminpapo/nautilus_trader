# Documentation: `crates/execution/src/matching_engine/engine.rs`
**Generated:** 2025-11-15T19:40:02.087316Z
**File Size:** 101429 bytes
**Extension:** .rs
**Type:** text
...
- **Path:** `crates/execution/src/matching_engine/engine.rs`
- **Size:** 101,429 bytes
- **Lines:** 2,630
- **Extension:** `.rs`
- **Type:** text
...
This file is located at `crates/execution/src/matching_engine/engine.rs` within the repository.

**Classes defined:** OrderMatchingEngine, Debug, OrderMatchingEngine

**Functions defined:** fmt, new, reset, set_fill_model, best_bid_price, best_ask_price, get_book, get_open_bid_orders, get_open_ask_orders, get_open_orders and 56 more
...
#### `get_open_bid_orders(&self)`


#### `get_open_ask_orders(&self)`


#### `get_open_orders(&self)`


#### `order_exists(&self, client_order_id: ClientOrderId)`
...
#### `process_trade_ticks_from_bar(&mut self, bar: &Bar)`


#### `process_quote_ticks_from_bar(&mut self, bar: &Bar)`


#### `process_trade_tick(&mut self, trade: &TradeTick)`


#### `process_status(&mut self, action: MarketStatusAction)`


#### `process_order(&mut self, order: &mut OrderAny, account_id: AccountId)`


#### `process_modify(&mut self, command: &ModifyOrder, account_id: AccountId)`


#### `process_cancel(&mut self, command: &CancelOrder, account_id: AccountId)`


#### `process_cancel_all(&mut self, command: &CancelAllOrders, account_id: AccountId)`


#### `process_batch_cancel(&mut self, command: &BatchCancelOrders, account_id: AccountId)`


#### `process_market_order(&mut self, order: &mut OrderAny)`


#### `process_market_order_with_protection(&mut self, order: &mut OrderAny)`


#### `process_limit_order(&mut self, order: &mut OrderAny)`


#### `process_market_to_limit_order(&mut self, order: &mut OrderAny)`


#### `process_stop_market_order(&mut self, order: &mut OrderAny)`


#### `process_stop_market_order_with_protection(&mut self, order: &mut OrderAny)`


#### `process_stop_limit_order(&mut self, order: &mut OrderAny)`


#### `process_market_if_touched_order(&mut self, order: &mut OrderAny)`


#### `process_limit_if_touched_order(&mut self, order: &mut OrderAny)`


#### `process_trailing_stop_order(&mut self, order: &mut OrderAny)`


#### `iterate(&mut self, timestamp_ns: UnixNanos)`
...
#### `iterate_orders(&mut self, timestamp_ns: UnixNanos, orders: &[PassiveOrderAny])`


#### `determine_limit_price_and_volume(&mut self, order: &OrderAny)`


#### `determine_market_price_and_volume(&self, order: &OrderAny)`


#### `fill_market_order(&mut self, order: &mut OrderAny)`
...
#### `update_limit_order(&mut self, order: &mut OrderAny, quantity: Quantity, price: Price)`
...
#### `update_trailing_stop_order(&mut self, order: &mut OrderAny)`


#### `update_protection_price(&mut self, order: &mut OrderAny)`
