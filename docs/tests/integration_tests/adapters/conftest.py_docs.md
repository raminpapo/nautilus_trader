# Documentation: `tests/integration_tests/adapters/conftest.py`
**Generated:** 2025-11-15T19:40:07.738700Z
**File Size:** 8173 bytes
**Extension:** .py
**Type:** text
...
This file is located at `tests/integration_tests/adapters/conftest.py` within the repository.

**Functions defined:** account_id, has_live_components_marker, clock, live_clock, trader_id, instance_id, msgbus, cache, portfolio, data_engine and 24 more
...
#### `has_live_components_marker(request)`
...
#### `portfolio(clock, cache, msgbus, account_state)`


#### `data_engine(msgbus, cache, clock, data_client)`


#### `exec_engine(request, event_loop, msgbus, cache, clock, exec_client)`
...
#### `mock_data_engine_process(mocker: MockerFixture, msgbus, data_engine)`


#### `mock_exec_engine_process(mocker: MockerFixture, msgbus, exec_engine)`


#### `strategy(trader_id, portfolio, msgbus, cache, clock)`
...
#### `venue_order_id(strategy)`


#### `trade_id(strategy)`


#### `components(data_engine, exec_engine, risk_engine, strategy)`


#### `_collect_events(msgbus, filter_types: tuple[type, ...] | None = None)`


#### `handler(event: Event)`


#### `events(msgbus: MessageBus)`


#### `fill_events(msgbus: MessageBus)`


#### `cancel_events(msgbus: MessageBus)`


#### `messages(msgbus: MessageBus)`


#### `account(account_state, cache)`
...
```python
from tests.integration_tests.adapters.conftest import account_id
```
...
**Directory:** `tests/integration_tests/adapters`
