# Documentation: `tests/unit_tests/msgbus/test_bus.py`
**Generated:** 2025-11-15T19:40:09.406028Z
...
This file is located at `tests/unit_tests/msgbus/test_bus.py` within the repository.

**Classes defined:** DummyListener

**Functions defined:** clock, trader_id, bus, test_instantiate_message_bus, test_endpoints_with_none_registered_returns_empty_list, test_topics_with_no_subscribers_returns_empty_list, test_subscriptions_with_no_subscribers_returns_empty_list, test_has_subscribers_with_no_subscribers_returns_false, test_register_adds_endpoint, test_deregister_removes_endpoint and 37 more
...
#### `test_instantiate_message_bus(bus, trader_id)`


#### `test_endpoints_with_none_registered_returns_empty_list(bus)`


#### `test_topics_with_no_subscribers_returns_empty_list(bus)`


#### `test_subscriptions_with_no_subscribers_returns_empty_list(bus)`


#### `test_has_subscribers_with_no_subscribers_returns_false(bus)`


#### `test_register_adds_endpoint(bus)`


#### `test_deregister_removes_endpoint(bus)`


#### `test_send_when_no_endpoint_at_address_logs_error(bus)`


#### `test_send_when_endpoint_at_address_sends_message_to_handler(bus)`


#### `test_request_when_endpoint_not_registered_logs_error(bus, clock)`


#### `test_request_response_when_correlation_id_registered_handles_response(bus, clock)`


#### `test_subscribe_then_returns_topics_list_including_topic(bus)`


#### `test_has_subscribers_when_subscribers_returns_true(bus)`


#### `test_subscribe_when_handler_already_subscribed_does_not_add_subscription(bus)`


#### `test_subscribe_then_subscriptions_list_includes_handler(bus)`


#### `test_subscribe_to_all_then_subscriptions_list_includes_handler(bus)`


#### `test_subscribe_all_when_handler_already_subscribed_does_not_add_subscription(bus)`


#### `test_unsubscribe_then_handler_not_in_subscriptions_list(bus)`


#### `test_unsubscribe_when_no_subscription_does_nothing(bus)`


#### `test_unsubscribe_from_all_returns_subscriptions_list_without_handler(bus)`


#### `test_unsubscribe_from_all_when_no_subscription_does_nothing(bus)`


#### `test_is_subscribed_lifecycle(bus)`


#### `handler(msg)`


#### `test_publish_with_no_subscribers_does_nothing(bus)`


#### `test_publish_with_subscriber_sends_to_handler(bus)`


#### `test_publish_with_multiple_subscribers_sends_to_handlers(bus)`


#### `test_publish_with_header_sends_to_handler(bus)`


#### `test_publish_with_header_sends_to_handler_after_published(bus)`


#### `test_publish_with_none_matching_header_then_filters_from_subscriber(bus)`


#### `test_publish_with_matching_subset_header_then_sends_to_subscriber(bus)`


#### `test_publish_with_both_channel_and_all_sub_sends_to_subscribers(bus)`


#### `test_subscribe_prior_to_publish_then_receives_message_on_topic(bus)`


#### `test_msgbus_for_system_events_using_component_id(bus)`


#### `test_is_matching_given_various_topic_pattern_combos(topic, pattern, expected)`


#### `test_duplicate_request_id_not_processed(bus, clock)`


#### `test_publish_question_mark_pattern(bus)`


#### `test_publish_invokes_handlers_in_priority_order(bus)`
...
#### `test_streaming_type_registration(bus)`


#### `test_add_listener_receives_byte_messages(bus)`


#### `__init__(self, closed=False)`
...
```python
from tests.unit_tests.msgbus.test_bus import DummyListener
```
